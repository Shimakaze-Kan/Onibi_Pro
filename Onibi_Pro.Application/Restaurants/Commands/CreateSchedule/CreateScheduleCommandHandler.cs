...
internal sealed class CreateScheduleCommandHandler : IRequestHandler<CreateScheduleCommand, ErrorOr<Success>>
{
    ...
    public async Task<ErrorOr<Success>> Handle(CreateScheduleCommand request, CancellationToken cancellationToken)
    {
        ...
        var schedule = Schedule.CreateUnique(request.Title, request.StartDate, request.EndDate, priority, employeeIds);
        ...
        var addScheduleResult = restaurant.AddSchedule(UserId.Create(_currentUserService.UserId), schedule.Value);

        if (addScheduleResult.IsError)
        {
            return addScheduleResult.Errors;
        }
